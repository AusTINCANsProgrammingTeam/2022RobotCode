...
public class AutonModes {
    ...
    public AutonModes(DriveBaseSubsystem d, ShooterSubsystem s, LimelightSubsystem l, CDSSubsystem c, IntakeSubsystem i) {
    ...
    private void initializeCommandGroups() {
        ...
        taxiCommand = new SequentialCommandGroup(
            new WaitCommand(Constants.delaytaxi),
            taxiRamseteCommands[0] 
        );
        
        oneballCommand = new SequentialCommandGroup(
            new WaitCommand(Constants.delaytaxi),
        ...
        twoballParallel = new ParallelDeadlineGroup(
            twoBallRamseteCommands[0],
            new IntakeForwardCommand(intakeSubsystem)
        );
        twoballCommand = new SequentialCommandGroup(
            new WaitCommand(Constants.delaytaxi),
            twoballParallel,
    ...
    private Trajectory[] getTrajectories(String routeString) {
        ...
        while(fileScanner.hasNext()) {
            ...
            Path path = Filesystem.getDeployDirectory().toPath().resolve(pathName); // goes to scr/main/deploy/paths
